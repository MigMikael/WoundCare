...
ap.add_argument("-s", "--suffix", required=True, help="image name suffix")
...
def process(image, model, suffix) :
    ...
    image_predict = predict_image('feature.txt', load_model(model))
    image_alpha_blening = alpha_blending( image_predict, image)
    
    cv2.imwrite('img/predict'+ str(suffix) +'.png' , image_predict)
    cv2.imwrite('img/alpha'+ str(suffix) + '.png', to_bgr(image_alpha_blening))
...
process(resize_image(read_color_img(image)), model, suffix)
